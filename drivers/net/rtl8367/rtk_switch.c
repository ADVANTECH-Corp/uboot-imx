...
#include <rtl8367/led.h>
#include <rtl8367/vlan.h>
#include <rtl8367/rtk_types.h>
#include <rtl8367/dal/dal_mgmt.h>
#include <rtl8367/dal/rtl8367c/rtl8367c_asicdrv_port.h>
...
void rtl8367_switch_init(switch_test_info_t test_info)
{
	...
	if(retVal != RT_ERR_OK)
	 {  
	 	printf("dal_rtl8367c_led_groupConfig_set fail\n");
	 	return;
	 }
	...
	if(retVal != RT_ERR_OK)
	{
	   printf("rtk_led_serialMode_set fail\n"); 
	   return;
	 }     
	...
	if(retVal != RT_ERR_OK)
	   printf("rtk_port_macForceLinkExt_set fail\n");
   
	txdelay=1;
	rxdelay=4;
	rtk_port_rgmiiDelayExt_set(EXT_PORT0,txdelay,rxdelay);
	if(retVal != RT_ERR_OK)
	   printf("rtk_port_rgmiiDelayExt_set fail\n");

	retVal=rtk_port_phyComboPortMedia_set(UTP_PORT4,PORT_MEDIA_FIBER);
	if(retVal != RT_ERR_OK)
	   printf("rtk_port_phyComboPortMedia_set fail\n");
	else
	   printf("switch booting …… ok\n");
/*
	//vlan init
	rtk_vlan_init();
	...
	*/
	if(test_info.test_port_number>=0&&test_info.test_port_number<=4)
	{
	   ...
	   printf("open switch mode,test lan number is %d\n",test_info.test_port_number);
	   if(test_info.test_full_100)
	   {
		   printf("set test lan mode is Full_100\n");
		   ...
		   if(retVal != RT_ERR_OK)
				printf("rtk_port_phyAutoNegoAbility_set fail\n");
	   }
	   retVal=RT_MAPPER->port_phyTestMode_set(test_info.test_port_number,1);
	   if(retVal != RT_ERR_OK)
			printf("port_phyTestMode_set fail\n");
	}
